...
if __name__ == '__main__':  # noqa: C901
    ...
    if args.trained_agent.endswith('.zip') and os.path.isfile(args.trained_agent):
        ...
        study = hyperparam_optimization(args.algo, create_model, create_env, n_trials=args.n_trials,
        ...
        study_name = (f"report_{env_id}_{args.n_trials}-trials-{n_timesteps}"
                       f"-{args.sampler}-{args.pruner}_{int(time.time())}.pkl")

        log_path = os.path.join(args.log_folder, args.algo, report_name)
        study_path = os.path.join(args.log_folder, args.algo, study_name)
